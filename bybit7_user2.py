"""
FinalSniperBotV6 - 클로드 4차 검수 최종 수정판
══════════════════════════════════════════════════
[3차 검수 반영 확인]
  ✅ Bug1(구): continue → processed_be 플래그 (XAUUSDT 스킵 방지)
  ✅ Bug2(구): start_price 재시작 복원 (get_executions + 반대체결 필터)
  ✅ Bug3(구): 헷지 종료 qty float 변환 + round 처리
  ✅ Bug4(구): sleep(3) 후 포지션 재조회 반영
  ✅ Improvement: calc_step 여유치 0.95 상향
  ✅ EMA50 수렴 정밀도: limit=200 상향
  ✅ EMA50 폴백 방어: ema50 <= 0 진입 금지

[4차 검수 신규 수정]
  🔴 BUG-1: if not main_p 블록 분리 오류
            진입 로직이 독립 if로 분리되어 포지션 있어도 추가 진입 주문 발생
            → 진입 로직 전체를 if not main_p 블록 안으로 이동
  🔴 BUG-2: ema50 <= 0 시 continue → if ema50 > 0 으로 교체
            continue가 for symbol 루프 자체를 건너뛰어
            포지션 있는 종목의 마틴게일 관리까지 스킵하는 버그
  🟡 NOTE:  296번줄 주석 '오래된 순 정렬' → 실제 최신순(reverse=True) 수정
